...
class spamsys(commands.Cog):
    ...
    async def on_message(self, ctx):
        ...
        if serverstats['Antispam'] is True:
            ...
            if warnings == warningmessage - 1:
                embed = discord.Embed(title=f":warning: WARNING",
                                      description=f"<@{ctx.author.id}>, You've been detected for spam. Repeated offences "
                                                  f"will get you automatically muted! You've been warned. ",
                                      colour=0xFFCC00)
        ...
        while True:
            await asyncio.sleep(int(os.environ["CLEARING"]))
            levelling.update_one({"guildid": ctx.guild.id, "id": ctx.author.id},
                                 {"$set": {"warnings": 0}})
    ...
    antispamstats.__doc__ = _("About:\nThe `antispamstats` command will allow you to see the current stats about AntiSpam "
